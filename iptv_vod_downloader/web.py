...
# In-memory store for queue items to sync between manager and API
...
def init_downloader():
    ...
    if download_manager is None:
        ...
        for item_data in queue_state_manager.load_items():
            ...
            # If it was active, we might want to resume it
            status = item_data.get("status")
            if status in {"queued", "downloading", "paused"}:
                # Re-create DownloadItem and add to manager
                item = DownloadItem(
...
class QueueAddRequest(BaseModel):
    items: List[Dict[str, Any]] # Kind, item_id, title, stream_url, etc.
...
async def remove_from_queue(queue_id: str):
    ...
    if download_manager.remove_item(queue_id):
        ...
        # Might be already completed or not found in manager but in our map
...
# Serve Frontend (static files will be in 'frontend/dist' after build)
