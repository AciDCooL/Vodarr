...
from .config import AppConfig, COMMON_USER_AGENTS, ConfigManager, QueueStateManager, UIStateManager, WindowState
...
class SeriesEpisodesDialog(tk.Toplevel):
    ...
    def __init__(
        ...
        sidebar_frame = ttk.Frame(content_frame, width=240)
        ...
        self.poster_label.pack(fill="x", pady=(0, 10))
        self.poster_image: Optional[ImageTk.PhotoImage] = None

        ttk.Label(sidebar_frame, text="Seasons").pack(anchor="w")
        season_list_frame = ttk.Frame(sidebar_frame)
        season_list_frame.pack(expand=True, fill="both")

        self.season_listbox = tk.Listbox(season_list_frame, exportselection=False, width=25)
        self.season_listbox.pack(side="left", expand=True, fill="both")
        self.season_listbox.bind("<<ListboxSelect>>", lambda _event: self._on_season_list_selected())

        season_scroll = ttk.Scrollbar(season_list_frame, orient="vertical", command=self.season_listbox.yview)
        self.season_listbox.configure(yscrollcommand=season_scroll.set)
        season_scroll.pack(side="right", fill="y")
        ...
        ttk.Button(button_frame, text="Select season", command=self._select_current_season).pack(side="left", padx=5)

        ttk.Button(button_frame, text="Queue selected", command=self.on_confirm).pack(side="left", padx=(20, 5))
        ttk.Button(button_frame, text="Queue season", command=self._add_current_season).pack(side="left", padx=5)
        ttk.Button(button_frame, text="Queue full series", command=self._add_entire_series).pack(side="left", padx=5)
    ...
    def _on_season_list_selected(self) -> None:
        season = self._get_selected_season()
        if season is not None:
            season_header_id = f"season-header-{season}"
            if self.tree.exists(season_header_id):
                self.tree.see(season_header_id)
                self.tree.selection_set(season_header_id)
    ...
    def _load_episodes(self) -> None:
        ...
        def populate() -> None:
            ...
            for season in sorted(self.season_episodes.keys()):
                ...
                self.season_listbox.insert(tk.END, label)
                self._season_labels[label] = season

            if self.season_listbox.size() > 0:
    ...
    def _get_selected_season(self) -> Optional[int]:
        ...
        if not selection:
            return None
        label = self.season_listbox.get(selection[0])
        return self._season_labels.get(label)
...
class IPTVApp(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        self.full_categories: Dict[str, List[Dict[str, Any]]] = {"movies": [], "series": []}
        self.items_map: Dict[str, Dict[str, Dict[str, Any]]] = {"movies": {}, "series": {}}
        self.queue_items: Dict[str, Dict[str, Any]] = {}

        self.error_count = 0
        self.last_error_at: Optional[float] = None
        self.error_history: List[float] = []  # timestamps of errors
    ...
    def _create_widgets(self) -> None:
        ...
        ttk.Label(self.config_frame, text="User-Agent").grid(row=3, column=0, sticky="w", padx=5, pady=5)
        ua_combo = ttk.Combobox(
            self.config_frame,
            textvariable=self.user_agent_var,
            values=list(COMMON_USER_AGENTS.keys()),
        )
        ua_combo.grid(row=3, column=1, columnspan=2, sticky="ew", padx=5, pady=5)

        def _on_ua_selected(_event: tk.Event) -> None:
            selected = self.user_agent_var.get()
            if selected in COMMON_USER_AGENTS:
                self.user_agent_var.set(COMMON_USER_AGENTS[selected])

        ua_combo.bind("<<ComboboxSelected>>", _on_ua_selected)
        ...
        self.error_stats_var = tk.StringVar(value="")
        ...
        # Error Timeline
        error_container = ttk.Frame(queue_toolbar)
        error_container.grid(row=0, column=4, sticky="e", padx=(20, 10))
        
        self.error_timeline_canvas = tk.Canvas(
            error_container, width=100, height=16, 
            bg="#f0f0f0", highlightthickness=1, highlightbackground="#d0d0d0"
        )
        self.error_timeline_canvas.pack(side="left", padx=(0, 5))
        
        ttk.Label(error_container, textvariable=self.error_stats_var, foreground="#cc0000").pack(side="left")

        ttk.Label(queue_toolbar, textvariable=self.queue_summary_var, foreground="gray").grid(row=0, column=6, sticky="e")

        columns = ("title", "kind", "status", "speed", "progress", "path", "error")
        ...
        self.queue_tree.column("progress", width=220, anchor="center")
    ...
    def _create_catalog_tab(self, kind: str, is_series: bool = False) -> ttk.Frame:
        ...
        ttk.Label(sidebar, text="Filter categories").pack(anchor="w", pady=(0, 5))
        filter_frame = ttk.Frame(sidebar)
        filter_frame.pack(fill="x", pady=(0, 5))

        category_filter_var = tk.StringVar()
        setattr(self, f"{kind}_category_filter_var", category_filter_var)
        filter_entry = ttk.Entry(filter_frame, textvariable=category_filter_var)
        filter_entry.pack(side="left", fill="x", expand=True)
        filter_entry.bind("<KeyRelease>", lambda _event, k=kind: self._on_category_filter(k))

        def _clear_cat_filter(k=kind, v=category_filter_var):
            v.set("")
            self._on_category_filter(k)

        ttk.Button(filter_frame, text="X", width=2, command=_clear_cat_filter).pack(side="right", padx=(2, 0))
        ...
        listbox = tk.Listbox(list_frame, exportselection=False, height=20, width=35)
    ...
    def _load_config_into_form(self) -> None:
        ...
        self.user_agent_var.set(self.current_config.user_agent)
    ...
    def _save_config(self) -> None:
        ...
        if not user_agent:
            ...
            self.user_agent_var.set(user_agent)
        ...
        self.download_manager.update_user_agent(user_agent)
    ...
    def _populate_categories(
        ...
        self.full_categories["movies"] = movie_categories
        self.full_categories["series"] = series_categories

        for kind in ("movies", "series"):
            filter_var: tk.StringVar = getattr(self, f"{kind}_category_filter_var")
            filter_var.set("")  # Clear filter on full refresh
            self._update_listbox_categories(kind)
            listbox: tk.Listbox = getattr(self, f"{kind}_listbox")
            listbox.selection_clear(0, tk.END)
            listbox.selection_set(0)
            self._load_items(kind, category_id="0")

    def _update_listbox_categories(self, kind: str) -> None:
        listbox: tk.Listbox = getattr(self, f"{kind}_listbox")
        filter_var: tk.StringVar = getattr(self, f"{kind}_category_filter_var")
        term = filter_var.get().strip().lower()

        categories = self.full_categories.get(kind, [])
        if term:
            categories = [
                c
                for c in categories
                if term in (c.get("category_name") or "").lower()
            ]
    ...
    def _on_category_filter(self, kind: str) -> None:
        self._update_listbox_categories(kind)
    ...
    def _populate_items(self, kind: str, items: List[Dict[str, Any]], request_token: Optional[int] = None) -> None:
        ...
        results_var: tk.StringVar = getattr(self, f"{kind}_results_var")
        results_var.set("Processing results...")
        ...
        def worker() -> None:
            ...
            tree_data: List[tuple[str, str, str, tuple[str, ...]]] = []
            ...
            for item in sorted_items:
                ...
                tree_data.append((identifier, name, year, tags))
            ...
            def update_ui() -> None:
                ...
                tree: ttk.Treeview = getattr(self, f"{kind}_tree")
                tree.delete(*tree.get_children())
                for identifier, name, year, tags in tree_data:
    ...
    def _format_speed(speed_bps: float) -> str:
        if speed_bps <= 0:
            return ""
        return f"{IPTVApp._format_size(speed_bps)}/s"

    def _queue_row_values(self, item: Dict[str, Any]) -> tuple[str, str, str, str, str, str, str]:
        progress = item.get("progress", 0.0)
        percent = f"{int(progress * 100)}%" if progress else "0%"
        status = item.get("status", "queued")
        speed = item.get("speed", 0.0) if status == "downloading" else 0.0

        downloaded = item.get("downloaded_bytes", 0)
        total = item.get("total_size", 0)

        if status in {"downloading", "completed", "paused", "stopped"}:
            if total > 0:
                progress_str = f"{self._format_size(downloaded)} / {self._format_size(total)} ({percent})"
            else:
                progress_str = f"{self._format_size(downloaded)} ({percent})"
        else:
            progress_str = percent
    ...
    def _draw_error_timeline(self) -> None:
        self.error_timeline_canvas.delete("all")
        if not self.error_history:
            return

        now = time.time()
        window_sec = 300  # Show last 5 minutes
        width = 100
        height = 16

        # Remove old errors from history (> 5 mins)
        self.error_history = [t for t in self.error_history if now - t < window_sec]

        for t in self.error_history:
            # Map time to x coordinate (right is newest)
            x = width - ((now - t) / window_sec * width)
            if 0 <= x <= width:
                self.error_timeline_canvas.create_line(x, 2, x, height - 2, fill="#ff0000", width=2)
    ...
    def _process_download_updates(self) -> None:
        ...
        # Periodic timeline redraw
        self._draw_error_timeline()
    ...
    def _update_queue_row(self, item: Dict[str, Any]) -> tuple[bool, bool]:
        ...
        if status == "failed" and status_changed:
            ...
            self.error_history.append(self.last_error_at)
            self._update_error_display()

        # Detect transient stream errors (retries / stalls)
        transient_errors = item.get("transient_errors", 0)
        previous_transient = previous.get("transient_errors", 0)
        if transient_errors > previous_transient:
            diff = transient_errors - previous_transient
            for _ in range(diff):
                self.error_count += 1
                self.last_error_at = time.time()
                self.error_history.append(self.last_error_at)
    ...
    def _update_error_display(self) -> None:
        if self.last_error_at:
            last_time = time.strftime("%H:%M:%S", time.localtime(self.last_error_at))
            self.error_stats_var.set(f"HTTP Errors: {self.error_count} (Last: {last_time})")
        self._draw_error_timeline()
